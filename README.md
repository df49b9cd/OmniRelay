...
```bash
dotnet build Polymer.slnx
dotnet test tests/Polymer.Tests/Polymer.Tests.csproj
```

Polymer targets `.NET 10` and pulls Hugo, gRPC, and JsonSchema.Net from NuGet. Tests expect loopback HTTP/2 support for gRPC scenarios.
...
`Polymer.Configuration` exposes spec interfaces (`ICustomInboundSpec`, `ICustomOutboundSpec`, `ICustomPeerChooserSpec`) so transports and peer choosers can be registered in DI and referenced by name.
...
- Peer leasing feeds `Polymer.Core.Peers` counters and the dispatcher readiness evaluator (`/readyz`).
