...
public sealed class ProtobufCodec<TRequest, TResponse> : ICodec<TRequest, TResponse>
    ...
    where TResponse : class, IMessage<TResponse>
{
    ...
    private readonly MessageDescriptor _requestDescriptor;
    private readonly MessageDescriptor _responseDescriptor;
    private readonly JsonParser _jsonParser;
    private readonly JsonFormatter _jsonFormatter;
    private readonly bool _allowJsonEncoding;
    ...
    {
        _requestParser = ResolveParser(requestParser);
        _responseParser = ResolveParser(responseParser);
        _requestDescriptor = ResolveDescriptor<TRequest>();
        _responseDescriptor = ResolveDescriptor<TResponse>();

        _jsonParser = jsonParser ?? JsonParser.Default;
        _jsonFormatter = jsonFormatter ?? JsonFormatter.Default;
        _allowJsonEncoding = allowJsonEncoding;

        Encoding = string.IsNullOrWhiteSpace(defaultEncoding)
    ...
    private Result<(EncodingKind Kind, string Encoding)> ResolveEncoding(string? encoding, string stage)
    {
        ...
        if (_allowJsonEncoding && ProtobufEncoding.IsJson(effectiveEncoding))
        {
