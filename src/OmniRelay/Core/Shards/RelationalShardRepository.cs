...
VALUES (@namespace, @shard, @version, @strategy, @owner, @previousOwner, @actor, @reason, @ticket, @delta, @metadata, @createdAt)";
        ...
        public const string StreamAll = @"SELECT h.id, r.namespace, r.shard_id, r.strategy_id, r.owner_node_id, r.leader_id, r.capacity_hint, r.status, r.version, r.checksum, r.updated_at, r.change_ticket, h.actor, h.reason, h.change_ticket, h.owner_node_id, h.previous_owner_node_id, h.ownership_delta_percent, h.metadata, h.created_at FROM shard_history h INNER JOIN shard_records r ON r.namespace = h.namespace AND r.shard_id = h.shard_id ORDER BY h.id";

        public const string StreamSince = @"SELECT h.id, r.namespace, r.shard_id, r.strategy_id, r.owner_node_id, r.leader_id, r.capacity_hint, r.status, r.version, r.checksum, r.updated_at, r.change_ticket, h.actor, h.reason, h.change_ticket, h.owner_node_id, h.previous_owner_node_id, h.ownership_delta_percent, h.metadata, h.created_at FROM shard_history h INNER JOIN shard_records r ON r.namespace = h.namespace AND r.shard_id = h.shard_id WHERE h.id > @position ORDER BY h.id";
    }
}
