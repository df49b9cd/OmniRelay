...
- DataPlane streaming paths now use Hugo result streaming helpers (`Result.MapStreamAsync` + `TapFailureEachAsync`) so channel consumers stop on the first failure/cancellation and propagate completion through transport calls without throwing exceptions.
