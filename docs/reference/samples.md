...
| ResourceLease mesh demo | `samples/ResourceLease.MeshDemo` | ResourceLease RPC mesh | Shows resource-neutral lease contracts, durable SQLite replication + deterministic store, backpressure hooks, diagnostics endpoints, and background workers exercising `resourcelease::*` procedures. |
