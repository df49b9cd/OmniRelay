...
- Shipped the relational persistence layer via `OmniRelay.ShardStore.Relational` with optimistic concurrency + audit history backed by the SQL schema in `eng/migrations/20241014-disc-003-shards.sql`, plus provider wrappers (`OmniRelay.ShardStore.Postgres`, `...Sqlite`, `...ObjectStorage`) to align with the repo naming pattern.
