...
public class GeneratedServiceIntegrationTests
{
    ...
    public async Task Unary_Http_Roundtrips()
    {
        ...
        try
        {
            var client = TestServiceYARPCore.CreateTestServiceClient(dispatcher, serviceName);
    ...
    public async Task Unary_Grpc_Roundtrips()
    {
        ...
        var dispatcher = new YARPCore.Dispatcher.Dispatcher(options);
        TestServiceYARPCore.RegisterTestService(dispatcher, new TestServiceImpl());
    ...
    private sealed class TestServiceImpl : TestServiceYARPCore.ITestService
    {
