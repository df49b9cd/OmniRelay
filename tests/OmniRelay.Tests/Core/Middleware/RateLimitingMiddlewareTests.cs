...
public sealed class RateLimitingMiddlewareTests
{
    ...
    public async Task UnaryOutbound_ReleasesPermitAfterSuccess()
    {
        ...
        var result1 = await middleware.InvokeAsync(request, CancellationToken.None, (UnaryOutboundDelegate)((req, token) => ValueTask.FromResult(Ok(Response<ReadOnlyMemory<byte>>.Create(ReadOnlyMemory<byte>.Empty)))));
        Assert.True(result1.IsSuccess);

        var result2 = await middleware.InvokeAsync(request, CancellationToken.None, (UnaryOutboundDelegate)((req, token) => ValueTask.FromResult(Ok(Response<ReadOnlyMemory<byte>>.Create(ReadOnlyMemory<byte>.Empty)))));
